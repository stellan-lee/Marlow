...
class _CapturingAgent:
    last_init = None
    last_persist_user_message = None
    ...
    def run_conversation(self, user_message, conversation_history=None, task_id=None, persist_user_message=None):
        type(self).last_persist_user_message = persist_user_message
        return {
            "final_response": "ok",
            "messages": [],
            "api_calls": 1,
...
@pytest.mark.asyncio
async def test_run_agent_accepts_enriched_event_without_undefined_event(monkeypatch, tmp_path):
    _install_fake_agent(monkeypatch)
    runner = _make_runner()
    ...
    _CapturingAgent.last_init = None
    _CapturingAgent.last_persist_user_message = None
    event = MessageEvent(
        text="hi",
        source=_make_source(),
        message_id="m1",
        raw_user_message="hi",
        external_conversation_snapshot=object(),
    )
    ...
    assert result["final_response"] == "ok"
    assert _CapturingAgent.last_persist_user_message == "hi"
